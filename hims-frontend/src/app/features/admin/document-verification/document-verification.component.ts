...
import { LucideAngularModule, Search, FileCheck, Check, X, Download, AlertCircle } from 'lucide-angular';
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
        <div class="relative w-64">
          <lucide-icon name="search" class="absolute left-3 top-2.5 h-4 w-4 text-slate-400"></lucide-icon>
          <input
            type="text"
            ...
            @for (doc of filteredDocs(); track doc.id) {
              <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <button (click)="download(doc.filePath)" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 flex items-center gap-1 font-medium">
...
export class DocumentVerificationComponent implements OnInit {
    ...
    download(fileName: string) {
        const url = this.docService.getDownloadUrl(fileName);
